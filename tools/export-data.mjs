...
write('forecasts.csv', csv(
  ['horizon', 'model', 'provider', 'api_model_id', 'asked_on', 'samples', 'ending_id', 'ending', 'family', 'extinction_tier',
  ...
  runs.flatMap(({ horizon, apiId, run }) => STATE_IDS.map(id => {
    ...
    return [horizon, run.label, run.provider, apiId, run.date, run.sampleCount, id, state.name, state.family, tierOf(state),
...
write('exposure.csv', csv(
  ['horizon', 'model', 'provider', 'api_model_id', 'asked_on', 'samples', 'humanity_gone_pct', 'might_perish_pct', 'total_exposure_pct', 'bootstrap_standard_error'],
  runs.map(({ horizon, apiId, run }) => {
    const sum = ids => ids.reduce((total, id) => total + run.probabilities[id], 0);
    return [horizon, run.label, run.provider, apiId, run.date, run.sampleCount,
...
write('rationales.csv', csv(
  ['horizon', 'model', 'api_model_id', 'asked_on', 'ending_id', 'ending', 'probability_pct', 'rationale'],
  runs.flatMap(({ horizon, apiId, run }) => STATE_IDS
    .filter(id => run.rationales?.[id])
    .map(id => [horizon, run.label, apiId, run.date, id, stateById.get(id).name, run.probabilities[id], run.rationales[id]]))
));
...
for (const file of readdirSync(join(root, 'runs')).filter(f => f.endsWith('.json')).sort()) {
  ...
  for (const sample of batch.samples || []) {
    for (const id of STATE_IDS) {
      sampleRows.push([horizon, batch.run_id, batch.model?.api_string, batch.model?.name, batch.asked_on,
        sample.sample, id, stateById.get(id).name, sample.answers?.['S' + id]?.value]);
    }
  }
}
write('samples.csv', csv(
  ['horizon', 'run_id', 'api_model_id', 'model', 'asked_on', 'sample', 'ending_id', 'ending', 'probability_pct'],
  sampleRows
));
...
for (const file of readdirSync(join(root, 'runs')).filter(f => f.endsWith('.json')).sort()) {
  ...
  qualityRows.push([
    horizon, batch.model?.name, batch.model?.api_string, batch.asked_on, kept, rejected,
...
write('quality.csv', csv(
  ['horizon', 'model', 'api_model_id', 'asked_on', 'samples_kept', 'answers_rejected',
