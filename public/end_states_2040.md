...
- Record the model ID, run date, sampling settings, horizon (`2040`), target year (`2040`), and question set (`end-states-2040-v2`) alongside each stored run.
...
2) Treat the states as exhaustive for this exercise. If the real future seems like a mixture, assign probability to the state that best describes the overall structure in place on December 31, 2040. Exception: if causally separated regions exhibit different states on that date, use The Diaspora.
